...
import com.android.fcclauncher.FocusHelper.PagedFolderKeyEventListener;
import com.android.fcclauncher.PageIndicator.PageMarkerResources;
import com.android.fcclauncher.Workspace.ItemOperator;
...
/*        float fraction = (direction == DragController.SCROLL_LEFT) ^ mIsRtl
        ...
        float fraction = (direction == DragController.SCROLL_TOP) ^ mIsRtl
                ? -SCROLL_HINT_FRACTION : SCROLL_HINT_FRACTION;
        int hint = (int) (fraction * getHeight());
        int scroll = getScrollForPage(getNextPage()) + hint;
        int delta = scroll - getScrollY();
        if (delta != 0) {
            mScroller.setInterpolator(new DecelerateInterpolator());
            mScroller.startScroll(0, getScrollY(), 0, delta, Folder.SCROLL_HINT_DURATION);
            invalidate();
        }
    }
...
/*    public void clearScrollHint() {
    ...
    public void clearScrollHint() {
        if (getScrollY() != getScrollForPage(getNextPage())) {
            snapToPage(getNextPage());
        }
    }
    /**
