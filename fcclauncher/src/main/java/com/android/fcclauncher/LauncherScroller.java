...
/*        for (int i = 0; i < NB_SAMPLES; i++) {
        ...
        for (int i = 0; i < NB_SAMPLES; i++) {
            ...
            while (true) {
                y = y_min + (y_max - y_min) / 2.0f;
                coef = 3.0f * y * (1.0f - y);
                ty = coef * ((1.0f - y) * P1 + y * P2) + y * y * y;
                if (Math.abs(ty - alpha) < 1E-5) break;
                if (ty > alpha) y_max = y;
                else y_min = y;
            }
            SPLINE_POSITION[i] = coef * ((1.0f - y) * START_TENSION + y) + y * y * y;
            ...
            while (true) {
                x = x_min + (x_max - x_min) / 2.0f;
                coef = 3.0f * x * (1.0f - x);
                dx = coef * ((1.0f - x) * START_TENSION + x) + x * x * x;
                if (Math.abs(dx - alpha) < 1E-5) break;
                if (dx > alpha) x_max = x;
                else x_min = x;
            }
            SPLINE_TIME[i] = coef * ((1.0f - x) * P1 + x * P2) + x * x * x;
        }
            ...
            switch (mMode) {
                case SCROLL_MODE:
                    ...
                    if (mInterpolator == null)
                        ...
                        y = mInterpolator.getInterpolation(y);
                ...
                case FLING_MODE:
                    ...
                    if (mCurrY == mFinalY && mCurrX == mFinalX) {
                        mFinished = true;
                    }
