...
import com.android.fcclauncher.util.LauncherEdgeEffect;
import com.android.fcclauncher.util.Thunk;
...
*/
    private int mFreeScrollMinScrollY = -1;
    private int mFreeScrollMaxScrollY = -1;
    ...
    private float mDragViewBaselineTop;
    ...
    // Edge effect
    private final LauncherEdgeEffect mEdgeGlowLeft = new LauncherEdgeEffect(getContext());
    private final LauncherEdgeEffect mEdgeGlowRight = new LauncherEdgeEffect(getContext());
/*    private final LauncherEdgeEffect mEdgeGlowTop = new LauncherEdgeEffect(getContext());
    private final LauncherEdgeEffect mEdgeGlowBottom = new LauncherEdgeEffect(getContext());*/
    private final LauncherEdgeEffect mEdgeGlowTop = new LauncherEdgeEffect(getContext());
    private final LauncherEdgeEffect mEdgeGlowBottom = new LauncherEdgeEffect(getContext());
...
/*            float x = (mLastMotionX - mDownMotionX) + (getScrollX() - mDownScrollX) +
            ...
            float x = (mLastMotionX - mDownMotionX);
            float y = (mLastMotionY - mDownMotionY) + (getScrollY() - mDownScrollY) +
                    (mDragViewBaselineTop - mDragView.getTop());
...
/*        int newX = 0;
        ...
        int newY = 0;
        if (0 <= mCurrentPage && mCurrentPage < getPageCount()) {
            newY = getScrollForPage(mCurrentPage);
        }
        scrollTo(0, newY);
...
/*            if (!mScroller.isFinished() &&
            ...
            if (!mScroller.isFinished() &&
                    (y > mFreeScrollMaxScrollY || y < mFreeScrollMinScrollY)) {
                forceFinishScroller();
            }

            y = Math.min(y, mFreeScrollMaxScrollY);
            y = Math.max(y, mFreeScrollMinScrollY);
        }
...
/*        boolean isXBeforeFirstPage = mIsRtl ? (x > mMaxScrollX) : (x < 0);
        ...
        boolean isYBeforeFirstPage = mIsRtl ? (y > mMaxScrollY) : (y < 0);
        boolean isYAfterLastPage = mIsRtl ? (y < 0) : (y > mMaxScrollY);
        if (isYBeforeFirstPage) {
            super.scrollTo(x, mIsRtl ? mMaxScrollY : 0);
            if (mAllowOverScroll) {
                mWasInOverscroll = true;
                if (mIsRtl) {
        ...
        } else if (isYAfterLastPage) {
            super.scrollTo(x, mIsRtl ? 0 : mMaxScrollY);
            if (mAllowOverScroll) {
                mWasInOverscroll = true;
                if (mIsRtl) {
                    overScroll(y);
...
/*            if (getScrollX() != mScroller.getCurrX()
            ...
            if (getScrollY() != mScroller.getCurrY()
                    || getScrollX() != mScroller.getCurrX()) {
                float scaleY = mFreeScroll ? getScaleY() : 1f;
                int scrollY = (int) (mScroller.getCurrY() * (1 / scaleY));
...
/*        int parentWidthSize = (int) (2f * maxSize);
        ...
        int parentWidthSize = (int) (2.5f * maxSize);
        int parentHeightSize = (int) (2.5f * maxSize);
...
/*                    childWidth = getViewportWidth() - mInsets.left - mInsets.right;
                    ...
                    childHeight = getViewportHeight() - mInsets.top - mInsets.bottom;/*sfcheck*/
                }
/*                if (referenceChildWidth == 0) {
                    referenceChildWidth = childWidth;
                }*/
                if (referenceChildHeight == 0) {
                    referenceChildHeight = childHeight;
                }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        final int childCount = getChildCount();
        if (DEBUG) Log.d(TAG, "childCount: " + childCount);
        ...
        /*horizontal scrolling version - original*/
...
/*        *//*sf - vertical scrolling version*//*
        int horizontalPadding = getPaddingLeft() + getPaddingRight();
        int verticalPadding = getPaddingTop() + getPaddingBottom();
        DeviceProfile profile = ((Launcher) getContext()).getDeviceProfile();

        LayoutParams lp = (LayoutParams) getChildAt(startIndex).getLayoutParams();
        LayoutParams nextLp;

        *//* Top and bottom screens can be dragged by this amount - shows the end of the scrollable screens *//*
        int topEdgeEffect = (int) (getViewportHeight() * 0.1);
        int bottomEdgeEffect = 0;
        int childTop = offsetY + topEdgeEffect;

        if (mPageScrolls == null || childCount != mChildCountOnLastLayout) {
            mPageScrolls = new int[childCount];
        }

        if (DEBUG) Log.d(TAG, "childTop: " + childTop);
        if (DEBUG) Log.d(TAG, "offsetY: " + offsetY);
        if (DEBUG) Log.d(TAG, "getViewportHeight(): " + getViewportHeight());
        if (DEBUG) Log.d(TAG, "getMeasuredHeight(): " + getMeasuredHeight());
        if (DEBUG) Log.d(TAG, "getPaddingTop(): " + getPaddingTop());
        if (DEBUG) Log.d(TAG, "mInsets.top: " + mInsets.top);
        if (DEBUG) Log.d(TAG, "mInsets.bottom: " + mInsets.bottom);

*//*        mPageScrolls[0] = - getPaddingTop() * 4;*//*
        for (int i = startIndex; i != endIndex; i += delta) {
            final View child = getPageAt(i);
            if (child.getVisibility() != View.GONE) {
                lp = (LayoutParams) child.getLayoutParams();
                int childLeft;
                if (lp.isFullScreenPage) {
                   childLeft = offsetX *//*+ profile.getOverviewModeButtonBarWidth()*//*;
                } else {
                    childLeft = offsetX + getPaddingLeft() + mInsets.left;
                    if (mCenterPagesHorizontally) {
                        childLeft += (getViewportWidth() - mInsets.left - mInsets.right - horizontalPadding - child.getMeasuredWidth()) / 2;
                    }
                }
                if (DEBUG) Log.d(TAG, "childTop2: " + childTop);
                ...
                *//*This is used to vertically center the 'scrollable' content of each page. We calculate top padding as a difference
                 * between usableHeight (height between the bottom of status bar and top of virtual keys minus top and bottom paddings)
                  * and the space occupied by the content itself (number of cells * height of cell)*//*
                int usableHeight = childHeight  - getPaddingTop() - getPaddingBottom();
                int allCellsUsedHeight = (int) (profile.inv.numRows * profile.cellHeightPx);
                int availableHeight = Math.max(0, usableHeight - allCellsUsedHeight);

                int scrollOffsetTop = lp.isFullScreenPage ? 0 : getPaddingTop();
                *//*With (childTop - offsetY) the content will be located in the top left corner
                * of the real screen and will be partially overlaid by the status bar.
                * mInsets.top - is the height of the status bar
                * availableHeight/2 - is the padding calculated above to vertically center the content*//*
                mPageScrolls[i] = childTop - offsetY - availableHeight/2 - mInsets.top;

                *//*We cannot scroll by a negative amount (which is the case with the top screen), e.g. move higher than the top edge of content,
                * so for the first (top) screen we 'extend' its dimensions by this 'virtual scroll' amount *//*
*//*                if (i == 0) {
                    child.layout(childLeft, childTop - mPageScrolls[i],
                            childLeft + childWidth, childTop + childHeight - mPageScrolls[i]);
                }*//*

                if (DEBUG) Log.d(TAG, "usableHeight: " + usableHeight + ", allCellsUsedHeight: " + allCellsUsedHeight +", childHeight: " + childHeight + ", getHeight(): " + getHeight() + ", getViewportHeight() : " + getViewportHeight() + ", availableHeight: " + availableHeight);
                if (DEBUG) Log.d(TAG, "mPageScrolls[i]: " + mPageScrolls[i] +  ", childHeight: " + childHeight + ", childTop: " + childTop + ", scrollOffsetTop: " + scrollOffsetTop + ", offsetY: " + offsetY + ", profile.getEdgeMarginPx()" +profile.getEdgeMarginPx());
*//*
                int scrollOffsetLeft = lp.isFullScreenPage ? 0 : getPaddingLeft();
                mPageScrolls[i] = childLeft - scrollOffsetLeft - offsetX;
*//*
                ...
                pageGap = getViewportHeight() - child.getMeasuredHeight();

                childTop += childHeight + pageGap + getChildGap();
            }
        }
        *//*sf - /vertical scrolling version*/

        /*sf - vertical scrolling version 2*/
        int horizontalPadding = getPaddingLeft() + getPaddingRight();
        int verticalPadding = getPaddingTop() + getPaddingBottom();
        DeviceProfile profile = ((Launcher) getContext()).getDeviceProfile();
        ...
        int topEdgeEffect = (int) (getViewportHeight() * 0.1);
        int bottomEdgeEffect = 0;
        int childTop = offsetY + mInsets.top /*+ topEdgeEffect*/;
...
@Thunk void updateMaxScrollY() {
    int childCount = getChildCount();
    if (childCount > 0) {
        final int index = mIsRtl ? 0 : childCount - 1;
        mMaxScrollY = getScrollForPage(index);
    } else {
        mMaxScrollY = 0;
    }
}
...
/*        int offset = getPageAt(index).getLeft() - getViewportOffsetX();*/
        int offset = getPageAt(index).getTop() - getViewportOffsetY();
    ...
    protected void getVisiblePages(int[] range) {
        ...
        if (pageCount > 0) {
/*            int viewportWidth = getViewportWidth();*//*sfcheck*/
...
/*                if (sTmpIntPoint[0] > viewportWidth) {*/
                if (sTmpIntPoint[0] > viewportHeight) {
                    if (range[0] == -1) {
                        continue;
                    } else {
                        break;
...
/*                sTmpIntPoint[0] = currPage.getMeasuredWidth();*/
                sTmpIntPoint[0] = currPage.getMeasuredHeight();
                Utilities.getDescendantCoordRelativeToParent(currPage, this, sTmpIntPoint, false);
                if (sTmpIntPoint[0] < 0) {
                    if (range[0] == -1) {
                        continue;
...
/*            int halfScreenSize = getViewportWidth() / 2;
            ...
            int halfScreenSize = getViewportHeight() / 2;
            int screenCenter = getScrollY() + halfScreenSize;
            ...
            final int topScreen = mTempVisiblePagesRange[0];
            final int bottomScreen = mTempVisiblePagesRange[1];
//            Log.d(TAG, "Visible topScreen = " + topScreen + ", bottomScreen = " + bottomScreen); /*sfproblem!*/
            if (topScreen != -1 && bottomScreen != -1) {
...
/*                    if (mForceDrawAllChildrenNextFrame ||
                    ...
                    if (mForceDrawAllChildrenNextFrame ||
                            (topScreen <= i && i <= bottomScreen && shouldDrawChild(v))) {
                        drawChild(canvas, v, drawingTime);
                    }
                }
...
/*        if (getPageCount() > 0) {
        ...
        if (getPageCount() > 0) {
            //https://chromium.googlesource.com/android_tools/+/18728e9dd5dd66d4f5edf1b792e77e/sdk/sources/android-19/android/widget/ScrollView.java
            super.draw(canvas);
                final int mPaddingLeft = 10;
                final int mPaddingRight = 10;
/*                final int scrollY = mScrollY;*/
                final int scrollY = 0;


                if (mEdgeGlowTop != null) {
                    if (DEBUG) Log.d(TAG, "mViewport1(canvas): " + mViewport);
                    final int restoreCount = canvas.save();
...
//                    canvas.rotate(270, width, 0);
                    if (DEBUG) Log.d(TAG, "canvas1: " + canvas.getClipBounds());
                ...
                if (mEdgeGlowBottom != null) {
                    final int restoreCount = canvas.save();
                    if (DEBUG) Log.d(TAG, "mViewport2(canvas): " + mViewport);
                    final int width = getWidth() - mPaddingLeft - mPaddingRight;
                    final int height = getHeight();
/*                    canvas.translate(-width + mPaddingLeft,
                            Math.max(getScrollRange(), scrollY) + height);*/
                    canvas.translate(-width + mPaddingLeft,
                            Math.max(0, scrollY) + height);
                    canvas.rotate(180, width, 0);
                    if (DEBUG) Log.d(TAG, "canvas2: " + canvas.getClipBounds());
...
/*        if (!mEdgeGlowBottom.isFinished()) {
            final int restoreCount = canvas.save();
            Rect display = mViewport;
            Log.d(TAG, "2_mViewport: " + display);
            Log.d(TAG, "2_canvas1: " + canvas.getClipBounds());
            canvas.translate(display.left, display.top + mPageScrolls[mIsRtl ? 0 : (getPageCount() - 1)]);
            Log.d(TAG, "2_canvas2: " + canvas.getClipBounds());
            canvas.rotate(270);
            Log.d(TAG, "2_canvas3: " + canvas.getClipBounds());
            getEdgeHorizontalPostion(sTmpIntPoint);
            canvas.translate(sTmpIntPoint[0] - display.left, -display.height());
            Log.d(TAG, "2_canvas5: " + canvas.getClipBounds());
            mEdgeGlowBottom.setSize(display.width(), sTmpIntPoint[1] - sTmpIntPoint[0]);
            if (mEdgeGlowBottom.draw(canvas)) {
                postInvalidateOnAnimation();
            }
            canvas.restoreToCount(restoreCount);
...
*//*                getEdgeHorizontalPostion(sTmpIntPoint);*//*
*//*                canvas.translate(display.top - sTmpIntPoint[1], 0);*//*
                canvas.translate(display.top - sTmpIntPoint[1], display.width() - display.left);
                *//*                mEdgeGlowLeft.setSize(sTmpIntPoint[1] - sTmpIntPoint[0], display.width());
                if (mEdgeGlowLeft.draw(canvas)) {
                    postInvalidateOnAnimation();
                }*//*
                mEdgeGlowTop.setSize(sTmpIntPoint[1] - sTmpIntPoint[0], display.width());*//*sfcheck*//*
                if (mEdgeGlowTop.draw(canvas)) {
                    postInvalidateOnAnimation();
                }
                canvas.restoreToCount(restoreCount);
...
*//*            if (!mEdgeGlowRight.isFinished()) {*//*
            if (!mEdgeGlowBottom.isFinished()) {
                ...
                Log.d(TAG, "canvas0: " + canvas.getWidth()+ ", " + canvas.getHeight() + ". " + canvas.getClipBounds());

                canvas.translate(display.left, display.top + mPageScrolls[mIsRtl ? 0 : (getPageCount() - 1)]);
                Log.d(TAG, "canvas1: " + canvas.getWidth()+ ", " + canvas.getHeight() + ". " + canvas.getClipBounds());

                *//*                canvas.rotate(90);*//*
                canvas.rotate(90);
                Log.d(TAG, "canvas2: " + canvas.getWidth()+ ", " + canvas.getHeight() + ". " + canvas.getClipBounds());
...
*//*                getEdgeHorizontalPostion(sTmpIntPoint);*//*
                ...
                *//*mEdgeGlowRight.setSize(sTmpIntPoint[1] - sTmpIntPoint[0], display.width());
                ...
                Log.d(TAG, "canvas3: " + canvas.getWidth()+ ", " + canvas.getHeight() + ". " + canvas.getClipBounds());
     ...
     * Returns the top and bottom position for the edge effect.
     */
    protected abstract void getEdgeHorizontalPostion(int[] pos);
    @Override
...
/*    protected boolean hitsPreviousPage(float x, float y) {
    ...
    protected boolean hitsPreviousPage(float x, float y) {
        if (mIsRtl) {
            return (y > (getViewportOffsetY() + getViewportHeight() -
                    getPaddingBottom() - mPageSpacing));
        }
        return (y < getViewportOffsetY() + getPaddingTop() + mPageSpacing);
    }
    /**
...
/*        sTmpRect.set(mViewport.left - mViewport.width() / 2, mViewport.top,
        ...
        sTmpRect.set(mViewport.left, mViewport.top - mViewport.height() / 2,
                mViewport.right, mViewport.bottom + mViewport.height() / 2);
//        Log.d(TAG,"sTmpRect.contains(x, y) = " + sTmpRect.contains(x, y) + ". " +  x + ", " + y + ". " + mViewport.top +"," + mViewport.bottom + ", " + mViewport.height() / 2);
...
/*                final int xDist = Math.abs(mScroller.getFinalX() - mScroller.getCurrX());
                ...
                final boolean finishedScrolling = (mScroller.isFinished() || yDist < mTouchSlop / 3);
...
/*        if (xMoved) {
        ...
        if (yMoved) {
            ...
            mLastMotionY = y;
            mLastMotionYRemainder = 0;
            mTouchY = getViewportOffsetY() + getScrollY();
            mSmoothingTime = System.nanoTime() / NANOTIME_DIV;
            onScrollInteractionBegin();
            pageBeginMoving();
        }
...
/*        final int halfScreenSize = getViewportWidth() / 2;*/
        final int halfScreenSize = getViewportHeight() / 2;
...
/*            totalDistance = v.getMeasuredWidth() + mPageSpacing;*/
            totalDistance = v.getMeasuredHeight() + mPageSpacing;
        } else {
        ...
        if (mPageScrolls == null || index >= mPageScrolls.length || index < 0) {
            ...
            if (!lp.isFullScreenPage) {
              /*  scrollOffset = mIsRtl ? getPaddingRight() : getPaddingLeft();*/
                scrollOffset = mIsRtl ? getPaddingBottom() : getPaddingTop();
            }

/*            int baselineX = mPageScrolls[index] + scrollOffset + getViewportOffsetX();
            return (int) (child.getX() - baselineX);*/
            int baselineY = mPageScrolls[index] + scrollOffset + getViewportOffsetY();
            return (int) (child.getY() - baselineY);
        }
    }
...
/*        int screenSize = getViewportWidth();
        ...
        invalidate();*/
        int screenSize = getViewportHeight();
        float f = (amount / screenSize);
        if (f < 0) {
            mEdgeGlowTop.onPull(-f);/*sfcheck*/
        } else if (f > 0) {
            mEdgeGlowBottom.onPull(f);
        } else {
            return;
        }
        invalidate();
...
/*        if (mIsRtl) {
        ...
        if (mIsRtl) {
            mFreeScrollMinScrollY = getScrollForPage(mTempVisiblePagesRange[1]);
            mFreeScrollMaxScrollY = getScrollForPage(mTempVisiblePagesRange[0]);
        } else {
            mFreeScrollMinScrollY = getScrollForPage(mTempVisiblePagesRange[0]);
            mFreeScrollMaxScrollY = getScrollForPage(mTempVisiblePagesRange[1]);
        }
    }
...
/*        if (mDragView != null) {
        ...
        return -1;*/
        if (mDragView != null) {
            int dragY = (int) (mDragView.getTop() + (mDragView.getMeasuredHeight() / 2)
                    + mDragView.getTranslationY());
            getFreeScrollPageRange(mTempVisiblePagesRange);/*sfcheck*/
            int minDistance = Integer.MAX_VALUE;
            int minIndex = indexOfChild(mDragView);
            for (int i = mTempVisiblePagesRange[0]; i <= mTempVisiblePagesRange[1]; i++) {
                View page = getPageAt(i);
                int pageY = (int) (page.getTop() + page.getMeasuredHeight() / 2);
                int d = Math.abs(dragY - pageY);
                if (d < minDistance) {
                    minIndex = i;
                    minDistance = d;
                }
...
/*                if (Math.abs(deltaX) >= 1.0f) {
                ...
                if (pageUnderPointIndex > -1 && pageUnderPointIndex != indexOfChild(mDragView)) {
                    ...
                    getFreeScrollPageRange(mTempVisiblePagesRange); /*sfcheck - hover*/
...
/*                                    int oldX = getViewportOffsetX() + getChildOffset(i);
                                    ...
                                    int oldY = getViewportOffsetY() + getChildOffset(i);
                                    int newY = getViewportOffsetY() + getChildOffset(i + shiftDelta);
...
*/
                ...
                final int pageHeight = getPageAt(mCurrentPage).getMeasuredHeight();
...
/*                    if (Math.abs(deltaX) > pageWidth * RETURN_TO_ORIGINAL_PAGE_THRESHOLD &&
                    ...
                    if (Math.abs(deltaY) > pageHeight * RETURN_TO_ORIGINAL_PAGE_THRESHOLD &&
                            Math.signum(velocityY) != Math.signum(deltaY) && isFling) {
                        returnToOriginalPage = true;
                    }
...
/*                    boolean isDeltaXLeft = mIsRtl ? deltaX > 0 : deltaX < 0;
                    ...
                    boolean isDeltaYTop = mIsRtl ? deltaY > 0 : deltaY < 0;
                    boolean isVelocityYTop = mIsRtl ? velocityY > 0 : velocityY < 0;
                    if (((isSignificantMove && !isDeltaYTop && !isFling) ||
                            (isFling && !isVelocityYTop)) && mCurrentPage > 0) {
                        finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage - 1;
                        snapToPageWithVelocity(finalPage, velocityY);
                    } else if (((isSignificantMove && isDeltaYTop && !isFling) ||
                            (isFling && isVelocityYTop)) &&
                            mCurrentPage < getChildCount() - 1) {
                        finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage + 1;
                        snapToPageWithVelocity(finalPage, velocityY);
                    } else {
...
*/
                    ...
                    int vY = (int) (-velocityY * scaleY);
                    int initialScrollY = (int) (getScrollY() * scaleY);

                    mScroller.setInterpolator(mDefaultInterpolator);
/*                    mScroller.fling(initialScrollX,
                            getScrollY(), vX, 0, Integer.MIN_VALUE, Integer.MAX_VALUE, 0, 0);
                    invalidate();*/
                    mScroller.fling(getScrollX(), initialScrollY,
                            0 , vY, 0, 0, Integer.MIN_VALUE, Integer.MAX_VALUE);
                    invalidate();
                }
...
/*        int minDistanceFromScreenCenter = Integer.MAX_VALUE;
        ...
        int minDistanceFromScreenCenter = Integer.MAX_VALUE;
        int minDistanceFromScreenCenterIndex = -1;
        int screenCenter = getViewportOffsetY() + getScrollY() + (getViewportHeight() / 2);
        final int childCount = getChildCount();
        for (int i = 0; i < childCount; ++i) {
            View layout = (View) getPageAt(i);
            int childHeight = layout.getMeasuredHeight();
            int halfChildHeight = (childHeight / 2);
            int childCenter = getViewportOffsetY() + getChildOffset(i) + halfChildHeight;
            int distanceFromScreenCenter = Math.abs(childCenter - screenCenter);
            if (distanceFromScreenCenter < minDistanceFromScreenCenter) {
                minDistanceFromScreenCenter = distanceFromScreenCenter;
                minDistanceFromScreenCenterIndex = i;
...
/*        int halfScreenSize = getViewportWidth() / 2;
        ...
        final int newY = getScrollForPage(whichPage);
...
/*        int newX = getScrollForPage(whichPage);
        ...
        int newY = getScrollForPage(whichPage);
...
public void scrollTop() {
    ...
    public boolean startReordering(View v) {
        ...
        mTempVisiblePagesRange[0] = 0;
        mTempVisiblePagesRange[1] = getPageCount() - 1;
//        Log.d(TAG, "getPageCount = " + getPageCount() + ",  mTempVisiblePagesRange[0] = " +  mTempVisiblePagesRange[0] + ",  mTempVisiblePagesRange[1] = " +  mTempVisiblePagesRange[1]);
        getFreeScrollPageRange(mTempVisiblePagesRange);
...
/*            mDragView.animate().scaleX(1.15f).scaleY(1.15f).setDuration(100).start();
            mDragViewBaselineLeft = mDragView.getLeft();*/
            mDragView.animate().scaleY(1.15f).scaleX(1.15f).setDuration(100).start();
            mDragViewBaselineTop = mDragView.getTop();
