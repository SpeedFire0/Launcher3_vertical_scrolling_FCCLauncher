...
public class DeviceProfile {
    ...
    // Overview mode
    /*for vertical launcher*/
    private final int overviewModeMinIconZoneWidthPx;
    private final int overviewModeMaxIconZoneWidthPx;
    private final int overviewModeBarItemHeightPx;
    private final int overviewModeBarSpacerHeightPx;
    /*for horizontal launcher - do not use*/
    ...
    private final int pageIndicatorWidthPx;
    ...
    public DeviceProfile(Context context, InvariantDeviceProfile inv,
        ...
        /*for vertical launcher - use*/
        pageIndicatorWidthPx =
                res.getDimensionPixelSize(R.dimen.dynamic_grid_page_indicator_height);
        overviewModeMinIconZoneWidthPx =
                res.getDimensionPixelSize(R.dimen.dynamic_grid_overview_min_icon_zone_width);
        overviewModeMaxIconZoneWidthPx =
                res.getDimensionPixelSize(R.dimen.dynamic_grid_overview_max_icon_zone_width);
        overviewModeBarItemHeightPx =
                res.getDimensionPixelSize(R.dimen.dynamic_grid_overview_bar_item_height);
        overviewModeBarSpacerHeightPx =
                res.getDimensionPixelSize(R.dimen.dynamic_grid_overview_bar_spacer_height);
...
/*        searchBarSpaceHeightPx = Math.min(heightPx,
                res.getDimensionPixelSize(R.dimen.dynamic_grid_search_bar_max_height));
        searchBarSpaceWidthPx = getSearchBarLeftOffset()
                + res.getDimensionPixelSize(R.dimen.dynamic_grid_search_bar_width);*/
...
/*        hotseatBarWidthPx = iconSizePx + 4 * edgeMarginPx;*/
    ...
    /** Returns the search bar top offset */
    private int getSearchBarLeftOffset() {
        if (isTablet && !isVerticalBarLayout()) {
            return 4 * edgeMarginPx;
        } else {
            return 2 * edgeMarginPx;
    ...
    public Rect getSearchBarBounds(boolean isLayoutRtl) {/*sfcheck2*/
        Rect bounds = new Rect();
        if (isLandscape && transposeLayoutWithOrientation) {
            if (isLayoutRtl) {
                bounds.set(availableWidthPx - searchBarSpaceHeightPx - edgeMarginPx, edgeMarginPx,
                        /*availableWidthPx*/searchBarSpaceHeightPx, availableHeightPx - edgeMarginPx);
            } else {
...
/*                padding.set(searchBarBounds.width(), edgeMarginPx,
    ...
    int getOverviewModeButtonBarWidth() {
        int zoneWidth = (int) (overviewModeIconZoneRatio * availableWidthPx);
        zoneWidth = Math.min(overviewModeMaxIconZoneWidthPx,
                Math.max(overviewModeMinIconZoneWidthPx, zoneWidth));
        return zoneWidth;
    }
...
/*            return new Rect(availableWidthPx - hotseatBarHeightPx, 0,
            ...
            return new Rect(0, 0,
                    hotseatBarHeightPx, availableHeightPx);
        } else {
...
/*            hotseat.findViewById(R.id.layout).setPadding(0, 2 * edgeMarginPx, 0, 2 * edgeMarginPx);*/
            hotseat.findViewById(R.id.layout).setPadding(0, 0, 0, 0);
        } else if (isTablet) {
...
/*            int visibleChildCount = getVisibleChildCount(overviewMode);
            ...
            int overviewButtonBarWidth = getOverviewModeButtonBarWidth();/*sfcheck*/
            lp = (FrameLayout.LayoutParams) overviewMode.getLayoutParams();
            lp.gravity = Gravity.CENTER_VERTICAL | Gravity.LEFT;
            int visibleChildCount = getVisibleChildCount(overviewMode);
            int totalItemHeight = visibleChildCount * overviewModeBarItemHeightPx;
            int maxHeight = totalItemHeight + (visibleChildCount - 1) * overviewModeBarSpacerHeightPx;

            lp.height = Math.min(availableHeightPx, maxHeight);
/*            lp.height = LayoutParams.MATCH_PARENT;*/
            lp.width = overviewButtonBarWidth;
            overviewMode.setLayoutParams(lp);

            // Set margin of all visible children except the last visible child
            for (int i = 0; i < visibleChildCount; i++) {
                View thisChild = overviewMode.getChildAt(i);
                ViewGroup.MarginLayoutParams clp = (ViewGroup.MarginLayoutParams) thisChild.getLayoutParams();
                clp.width = lp.MATCH_PARENT;
                clp.height = lp.WRAP_CONTENT;
                thisChild.setLayoutParams(clp);
            }
        }
/*            if (lp.height > totalItemHeight && visibleChildCount > 1) {
                // We have enough space. Lets add some margin too.
                int margin = (lp.height - totalItemHeight) / (visibleChildCount-1);
                View lastChild = null;
