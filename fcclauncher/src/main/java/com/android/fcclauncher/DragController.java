...
/*    static final int SCROLL_LEFT = 0;
    ...
    private int[] getClampedDragLayerPos(float x, float y) {
        ...
        mTmpPoint[0] = (int) Math.max(mDragLayerRect.left, Math.min(x, mDragLayerRect.right - 1));/*sfcheck*/
...
/*        final int forwardDirection = mIsRtl ? SCROLL_RIGHT : SCROLL_LEFT;
        ...
        final int forwardDirection = mIsRtl ? SCROLL_BOTTOM : SCROLL_TOP;
        final int backwardsDirection = mIsRtl ? SCROLL_TOP : SCROLL_BOTTOM;
...
//            Log.d(TAG, "mScrollZone = " + (y > mScrollView.getHeight() - mScrollZone) + ", y = " + y + ", mScrollView.getHeight() = " + mScrollView.getHeight() + ", mScrollZone = " + mScrollZone);
            if (mScrollState == SCROLL_OUTSIDE_ZONE) {
                mScrollState = SCROLL_WAITING_IN_ZONE;
 ...
 /*           if ((dragLayerX < mScrollZone) || (dragLayerX > mScrollView.getWidth() - mScrollZone)) {*/
            if ((dragLayerY < mScrollZone) || (dragLayerY > mScrollView.getHeight() - mScrollZone)) {
                mScrollState = SCROLL_WAITING_IN_ZONE;
    ...
    private PointF isFlingingToDelete(DragSource source) {
        ...
        if (mVelocityTracker.getYVelocity() < mFlingToDeleteThresholdVelocity) {
            ...
            if (theta <= Math.toRadians(MAX_FLING_DEGREES)) { /*sfcheck*/
                return vel;
            }
        }
        return null;
